...
public class FormStorage {
    ...
    public static void sendShopForm(Player player){
        ...
        if(kg == 0){
            ...
            }else if(item.getId() == ItemID.FISHING_ROD){
                content = "&l&fKhông thể bán cần câu!";
            }else {
                price = 1;
        ...
        form.setHandler((p , response) -> {
            if(response.getToggle("xacnhan").getValue() && item.getId() != ItemID.FISHING_ROD){
                ...
                p.getLevel().addSound(p.getPosition(), Sound.FIREWORK_SHOOT);
            }
        });
        form.send(player);
    }
    ...
    public static void sendUpgradeForm(Player player){
        ...
        form.setHandler((p, response) -> {
            if(response.getToggle("cancan").getValue()
                    && finalNext_level < 301 && item.getId() == ItemID.FISHING_ROD){
                ...
                if(ecoapi.myMoney(p) >= finalPrice){
                    ...
                    p.getLevel().addSound(p.getPosition(), Sound.HIT_ANVIL);
